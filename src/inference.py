...
def get_paragraphs(input_text, retriever):
    return {page.page_content.strip() for page in retriever.invoke(input_text)}
...
def compute_relevance_scores(token_ids, log_probs, rel_tokens):
    rel_score_dict = {}
    relevance_indices = [idx for idx, tok in enumerate(
        token_ids) if tok in rel_tokens.values()]

    for idx in relevance_indices:
        for token, token_id in rel_tokens.items():
            prob = log_probs[idx][token_id].logprob if token_id in log_probs[idx] else -100
            rel_score_dict[token] = np.exp(prob)

    return rel_score_dict


def run_relation_generation_batch(model, prompt,  context, topic_entity, hypo=True, use_1hop=True, income=None, rel_tokens=None, embeddings=None, all_db=None, sampling_params=None, ):
    final_preds = []
    overall_scores = []
    final_contexts = []
    paragraph_set = set()
    ...
    paragraph_set.update(get_paragraphs(input_text, retriever))
    if hypo:
        hypo_rel = model.generate(
            prompt + retrieval_token, sampling_params)[0].outputs[0].text
        pattern = r'(\w+\.\w+\.\w+)\[(.*?)\]'
        hypo_rel = hypo_rel.split('[Retrieve Entity]')[
            0] if '[Retrieve Entity]' in hypo_rel else hypo_rel
        matches = re.findall(pattern, hypo_rel)
        relevant_relations = ' '.join(k for k, v in matches if v in {
                                      'Fully Relevant', 'Partially Relevant'})
        if relevant_relations:
            paragraph_set.update(get_paragraphs(relevant_relations, retriever))
        paragraph_set.discard(income)
        aug_prompts = [
            f"<paragraph>{';'.join(paragraphs)}</paragraph>"
            for paragraphs in chunk_paragraphs(list(paragraph_set), chunk_size=5)
        ]
    ...
    for p_id, pred in enumerate(preds):
        ...
        rel_score_dict = compute_relevance_scores(
            pred_token_ids, pred_log_probs, rel_tokens)

        relevance_score = (
            rel_score_dict.get('[Fully Relevant]', 0) +
            rel_score_dict.get('[Partially Relevant]', 0)
        ) / max(sum(rel_score_dict.values()), 1)
...
def run_entity_generation_batch(model, prompt, topic_entities, context, sampling_params=None):
    # 初始化变量
    # 初始化变量
...
def main():
    ...
    parser.add_argument('--model_name', type=str,
                        default='/media/disk2/llama_factory/generation_0122_no_mask/')
    ...
    count = 0
    correct_ids = []
    for index in range(11, len(test_data)):
        hit = 0
            ...
            if curr_depth-1 in levels:
                for node in levels[curr_depth-1]:
                    ...
                    if "<|eot_id|>" in curr_pred or 'No Retrieval' in curr_pred:
                        continue
                    cur_prompt = prediction_tree[node]["prompt"] + \
                    ...
                    if "Retrieve Entity" in curr_pred.split('[')[-1]:
                        ...
                        preds, scores, next_entities, contexts, overall_scores, income_rel = run_entity_generation_batch(
                            model, cur_prompt, topic_entity, context, sampling_params)
                    ...
                    if "Retrieve Relation" in curr_pred.split('[')[-1]:
                        ...
                        preds, scores, contexts = run_relation_generation_batch(
                            model, cur_prompt,  context=context, topic_entity=topic_entity, hypo=True, income=income_rel,
        ...
        if args.save_output and len(prediction_trees) == 1:
            print('saving logging res')
            save_to_json(prediction_trees, args.output_file)
            prediction_trees = []
    if args.save_output and len(prediction_trees):
        save_to_json(prediction_trees, args.output_file)
